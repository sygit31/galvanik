...
Public Class erp_galv_prosesDAO
    ...
    Public Function get_galv_proses() As List(Of erp_galv_proses)
        Dim hasil As New List(Of erp_galv_proses)
        Try
            Dim rdr As OracleDataReader
            Dim cmd As New OracleCommand("SELECT gp.ID, kp.NMR, gp.KODE_PROSES " & _
                                         "FROM erp_galv_proses gp " & _
                                         "INNER JOIN erp_ppic_kp_detail kpd ON gp.ID_KP_DETAIL = kpd.ID " & _
                                         "INNER JOIN erp_ppic_kp kp ON kpd.ID_KP = kp.ID " & _
                                         "ORDER BY gp.ID", oraconn)
            ...
            If rdr.HasRows Then
                While rdr.Read
                    Dim S As New erp_galv_proses
                    S.id = rdr("ID").ToString
                    S.nmr = rdr("NMR").ToString
                    S.kode_proses = rdr("KODE_PROSES").ToString
                    hasil.Add(S)
                End While
            End If
        ...
        Catch ex As Exception
            If oraconn.State = ConnectionState.Open Then
                oraconn.Close()
            End If
            errorLog("erp_galv_prosesDAO-get_galv_proses", ex.Message, ex.ToString)
            Return hasil
        End Try
    End Function
